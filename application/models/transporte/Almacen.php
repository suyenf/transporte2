...
//  }
    ...
    public function registro_carga($data) {
     ...
     public function crear_orden($orden, $tipo_armador, $cod_armador, $tipo_cliente, $cod_cliente, $descripcion, $fecha_inicio_estimada, $fecha_inicio_real, $fecha_fin_estimada, $fecha_fin_real) {
        $r = FALSE;
        
        $qo = $this->listar_orden($orden); // busca una orden con igual codigo
        if (count($qo)===0) { // SE VERIFICA QUE NO EXISTA NINGUNA ORDEN CON ESE NUMERO
            $this->db
                    ->set('HOPHOP', $orden)
                    ->set('CABARC', $tipo_cliente)
                    ->set('AUARMA', trim($cod_cliente))
                    ->set('CAARMA', $tipo_armador)
                    ->set('AUBARC', trim($cod_armador))
                    ->set('HOPDST', $descripcion)
                    ->set('HOPFIE', (string) $fecha_inicio_estimada) //funcion verf_fec para validar a fecha
                    ->set('HOPFIR', (string) $fecha_inicio_real)
                    ->set('HOPFFE', (string) $fecha_fin_estimada)
                    ->set('HOPFFR', (string) $fecha_fin_real)
                    ->set('HOPFEP', (string) date('Ymd'))
                    ->set('HOPSTS', 'A')
                    ->insert($this->tabla);
            $r = TRUE;
        }
